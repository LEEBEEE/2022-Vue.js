          ...
          <div class="row">
            ...
            <div class="col-lg-3 col-md-4 col-sm-2" :key="item.id" v-for="item in productImage.filter( c => c.type === 1 )">
              <div class="position-relative">
                ...
                <div class="position-absolute top-0 end-0" style="cursor:pointer;" @click="deleteImg(item)">X</div>
              </div>
            </div>
...
export default {
  ...
  methods: {
    ...
    async uploadFile(file, type) {
      console.log(file);
      const image = await this.$base64(file[0]);
      const formData = { image }
      const { error } = await this.$post(`/api/upload/${this.productDetail.id}/${type}`, formData);
      console.log(error);
    },
    async deleteImg({id, product_id, type, path}) {
      console.log(id, product_id, type, path);
      // console.log(id);
      // const result = await this.$delete(`/api/productImgDel/${id}`);
      const result = await this.$delete(`/api/productImgDel/${id}/${product_id}/${type}/${path}`);
      console.log(result);
    }
  }
}
