...
class ApiModel extends Model {
    public function getCategoryList() {
        $sql = " SELECT * FROM t_category ORDER BY cate1, cate2, cate3 ";
        $stmt = $this->pdo->prepare($sql);
        $stmt->execute();
        return $stmt->fetchAll(PDO::FETCH_OBJ);
    }

    public function productIns(&$param) {
    ...
    public function productImgIns(&$param) {
    ...
    public function productImgList(&$param) {
    ...
    // public function selImgOne(&$param) {
    //     $sql = "SELECT * FROM t_product_img WHERE id = :img_id";
    //     $stmt = $this->pdo->prepare($sql);
    //     $stmt->bindValue(':img_id', $param["img_id"]);
    //     $stmt->execute();
    //     return $stmt->fetch(PDO::FETCH_OBJ);
    // }

    public function productImgDel(&$param) {
        $sql = "DELETE FROM t_product_img WHERE id = :img_id";
        $stmt = $this->pdo->prepare($sql);
        $stmt->bindValue(':img_id', $param["img_id"]);
        $stmt->execute();
        return $stmt->rowCount();
    ...
    // foreign key에 on delete cascade 추가해서 t_product row를 삭제하면 t_product_img도 같이 삭제
